    ...
    <nav className={`fixed w-full  z-20 md:bg-opacity-90 transition duration-300 ease-in-out ${!top ? ' border-black border-b-1 backdrop-blur-sm ' : ''} flex items-center justify-between flex-wrap p-6`}>
      ...
      >
        <div className="text-sm lg:flex-grow">
          ...
          <Link className={`text-lg font-semibold hover:font-bold block mt-4 mx-4 lg:inline-block lg:mt-0  mr-4 ${activePage == 'Products' ? 'border-b-4 rounded border-blue-800' : ''} `} href={"/product"}>Products</Link>
